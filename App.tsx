...
const SettingsView: React.FC<{ creds: Credentials, setCreds: (c: Credentials) => void, profile: AppProfile, onChangeProfile: (id: string) => void }> = ({ creds, setCreds, profile, onChangeProfile }) => (
  <div className="max-w-4xl space-y-12 animate-in fade-in duration-500">
    ...
    <div className="bg-slate-900/30 border border-white/5 p-12 rounded-[50px] space-y-8 backdrop-blur-3xl shadow-2xl"><div className="flex items-center gap-4 text-red-500"><Globe /><h4 className="text-2xl font-black text-white uppercase italic">Platform Sync Protocol</h4></div><div className="space-y-4"><label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4">Secure Master API Key</label><input type="text" value={creds.blogApiKey} onChange={e => setCreds({ ...creds, blogApiKey: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-3xl px-8 py-6 text-white font-mono focus:border-red-600/50 outline-none" /></div></div>
